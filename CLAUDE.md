...
- **modapi.go**: Shared Modrinth/CurseForge query layer — `SearchBothSources` (TUI live add-mod search), `CLISearch`/`CLIModInfo` (`search`/`mod-info` subcommands, the agent's lookup tools). All requests go through `cachedGET`, a 15-min disk cache under `~/.cache/packwiz-tui/api/` shared by TUI and CLI. CurseForge uses `CURSEFORGE_API_KEY` against the official API when set, else the keyless curse.tools mirror
- **modicon.go** / **kittyimg.go**: image blocks for the add-mod popup — `fetchImageBlock` returns kitty-graphics placeholder cells (full-res, Unicode-placeholder protocol so Bubble Tea repaints are safe; kitty/ghostty via env detection) or half-block (▀) pixel art sized to the pane as fallback; nothing on no-color terminals
